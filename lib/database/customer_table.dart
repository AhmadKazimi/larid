...
class CustomerTable {
  ...
  Future<void> createOrUpdateSalesRepCustomers(
    List<CustomerEntity> customers,
  ) async {
    final batch = db.batch();

    for (final customer in customers) {
      batch.insert(salesrepCustomerTableName, {
        'customerCode': customer.customerCode,
        'customerName': customer.customerName,
        'address': customer.address,
        'contactPhone': customer.contactPhone,
    ...
    if (updatedRows == 0) {
      ...
      if (customerExists) {
        ...
        try {
          ...
          print(
            'Raw update attempted for ending visit for customer code: $customerCode',
          );
        } catch (e) {
  ...
  // Get the active customer visit for API reporting
  Future<Map<String, dynamic>?> getActiveCustomerVisit() async {
    try {
      final List<Map<String, dynamic>> maps = await db.query(
        salesrepCustomerTableName,
        where: 'visitStartTime IS NOT NULL AND visitEndTime IS NOT NULL',
        orderBy: 'visitEndTime DESC',
        limit: 1,
      );
      ...
      return {
        'customer_code': maps[0]['customerCode'],
        'customer_name': maps[0]['customerName'],
        'visit_start_time': maps[0]['visitStartTime'],
        'visit_end_time': maps[0]['visitEndTime'],
      };
    } catch (e) {
      print('Error getting active customer visit: $e');
      return null;
    }
  }
    ...
    try {
      ...
      try {
        ...
        if (visitEndTime.contains('T')) {
          ...
          final bool isWithin24Hours = lastVisitDate.isAfter(
            twentyFourHoursAgo,
          );
